...
codex-claw's tools configuration is located in the `tools` field of `config.json`.
...
Before tool results are sent to the LLM, codex-claw can filter sensitive values (API keys, tokens, secrets) from the output. This prevents the LLM from seeing its own credentials.
...
If `range` is omitted, codex-claw performs an unrestricted search.
...
By default, codex-claw blocks the following dangerous commands:
...
The exec guard only validates the top-level command sent to codex-claw. It does **not** recursively inspect child
