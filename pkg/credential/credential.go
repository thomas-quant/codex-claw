...
//   - Encrypted:   "enc://<base64>"     → AES-256-GCM decrypt via CODEX_CLAW_KEY_PASSPHRASE
...
//  2. CODEX_CLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/codex-claw_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
const PassphraseEnvVar = "CODEX_CLAW_KEY_PASSPHRASE"

// PassphraseProvider is the function used to retrieve the passphrase for enc://
// credential decryption. It defaults to reading CODEX_CLAW_KEY_PASSPHRASE from the
...
//
// passphrase is required (CODEX_CLAW_KEY_PASSPHRASE value).
// sshKeyPath is the SSH private key file to use; pass "" to auto-detect via
// CODEX_CLAW_SSH_KEY_PATH env var or ~/.ssh/codex-claw_ed25519.key.
...
//   - exact match with CODEX_CLAW_SSH_KEY_PATH env var
//   - within the CODEX_CLAW_HOME env var directory
//   - within ~/.ssh/
func allowedSSHKeyPath(path string) bool {
	...
	// Exact match with CODEX_CLAW_SSH_KEY_PATH.
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set CODEX_CLAW_SSH_KEY_PATH or place key at ~/.ssh/codex-claw_ed25519.key)")
	}
	if !allowedSSHKeyPath(sshKeyPath) {
		return nil, fmt.Errorf(
			"credential: SSH key path %q is not in an allowed location (CODEX_CLAW_SSH_KEY_PATH, CODEX_CLAW_HOME, or ~/.ssh/)",
			sshKeyPath,
		)
	}
...
//  2. CODEX_CLAW_SSH_KEY_PATH env var
//  3. ~/.ssh/codex-claw_ed25519.key (auto-detection)
//
...
// findDefaultSSHKey returns the codex-claw-specific SSH key path if it exists.
