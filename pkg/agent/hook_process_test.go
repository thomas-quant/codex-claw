...
func TestProcessHook_HelperProcess(t *testing.T) {
	if os.Getenv("CODEX_CLAW_HOOK_HELPER") != "1" {
		return
	}
...
func processHookHelperEnv(mode, eventLog string) []string {
	env := []string{
		"CODEX_CLAW_HOOK_HELPER=1",
		"CODEX_CLAW_HOOK_MODE=" + mode,
	}
	if eventLog != "" {
		env = append(env, "CODEX_CLAW_HOOK_EVENT_LOG="+eventLog)
	}
	return env
}
...
func runProcessHookHelper() error {
	mode := os.Getenv("CODEX_CLAW_HOOK_MODE")
	eventLog := os.Getenv("CODEX_CLAW_HOOK_EVENT_LOG")
