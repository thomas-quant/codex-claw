...
import (
	...
	"github.com/thomas-quant/codex-claw/pkg/audio/asr"
...
func (al *AgentLoop) runTurn(ctx context.Context, ts *turnState) (turnResult, error) {
	...
	threadControlReq := providers.InteractiveThreadControlRequest{
		SessionKey: ts.sessionKey,
		AgentID:    ts.agent.ID,
		Channel:    ts.channel,
		ChatID:     ts.chatID,
...
turnLoop:
	for ts.currentIteration() < ts.agent.MaxIterations || len(pendingMessages) > 0 || func() bool {
		...
		var llmStreamer bus.Streamer
		var streamedText strings.Builder
		cancelInteractiveStream := func() {
			if llmStreamer == nil {
				return
			}
			cancelCtx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
			llmStreamer.Cancel(cancelCtx)
			cancel()
			llmStreamer = nil
			streamedText.Reset()
		}
		finalizeInteractiveStream := func(content string) {
			if llmStreamer == nil {
				return
			}
			finalizeCtx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
			if err := llmStreamer.Finalize(finalizeCtx, content); err != nil {
				logger.WarnCF("agent", "Failed to finalize interactive stream", map[string]any{
					"agent_id":  ts.agent.ID,
					"channel":   ts.channel,
					"chat_id":   ts.chatID,
					"iteration": iteration,
		...
		callLLM := func(messagesForCall []providers.Message, toolDefsForCall []providers.ToolDefinition) (*providers.LLMResponse, error) {
			...
			if interactiveProvider, ok := activeProvider.(providers.InteractiveProvider); ok {
				...
				if al.bus != nil && ts.channel != "" && ts.chatID != "" {
					if llmStreamer == nil {
						if streamer, ok := al.bus.GetStreamer(providerCtx, ts.channel, ts.chatID); ok {
					...
					if llmStreamer != nil {
						...
						onChunk = func(chunk string) {
							...
							if err := llmStreamer.Update(providerCtx, streamedText.String()); err != nil {
								logger.WarnCF("agent", "Failed to update interactive stream", map[string]any{
									"agent_id":  ts.agent.ID,
									"channel":   ts.channel,
									"chat_id":   ts.chatID,
									"iteration": iteration,
		...
		if len(response.ToolCalls) == 0 || gracefulTerminal {
			...
			if strings.TrimSpace(responseContent) != "" {
				finalizeInteractiveStream(responseContent)
			} else {
				cancelInteractiveStream()
			}
...
func (al *AgentLoop) buildCommandsRuntime(agent *AgentInstance, opts *processOptions) *commands.Runtime {
	...
	if agent != nil {
		...
		if runtimeController, ok := agent.Provider.(providers.InteractiveRuntimeController); ok && opts != nil {
			threadReq := providers.InteractiveThreadControlRequest{
				SessionKey: opts.SessionKey,
				AgentID:    agent.ID,
				Channel:    opts.Channel,
				ChatID:     opts.ChatID,
			}
			withRuntimeContext := func() (context.Context, context.CancelFunc) {
				return context.WithTimeout(context.Background(), 10*time.Second)
			}
			rt.ListModels = func() []commands.ModelInfo {
				ctx, cancel := withRuntimeContext()
				defer cancel()

				models, err := runtimeController.ListModels(ctx)
				if err != nil {
					logger.WarnCF("agent", "Failed to list interactive runtime models", map[string]any{
						"agent_id": agent.ID,
						"error":    err.Error(),
					})
					return nil
				...
				out := make([]commands.ModelInfo, 0, len(models))
				for _, model := range models {
					out = append(out, commands.ModelInfo{
						Name:     model.ID,
			...
			rt.ReadStatus = func() (commands.StatusSnapshot, bool) {
				...
				status, err := runtimeController.ReadThreadStatus(ctx, threadReq)
				if err != nil {
					logger.WarnCF("agent", "Failed to read interactive runtime status", map[string]any{
						"agent_id": agent.ID,
						"error":    err.Error(),
			...
			rt.SetModel = func(value string) (string, error) {
				ctx, cancel := withRuntimeContext()
				defer cancel()
				return runtimeController.SetThreadModel(ctx, threadReq, strings.TrimSpace(value))
			}
			rt.SetThinking = func(value string) (string, error) {
				ctx, cancel := withRuntimeContext()
				defer cancel()
				return runtimeController.SetThreadThinking(ctx, threadReq, strings.TrimSpace(value))
			}
			rt.ToggleFast = func() (bool, error) {
				ctx, cancel := withRuntimeContext()
				defer cancel()
				return runtimeController.ToggleThreadFast(ctx, threadReq)
			}
			rt.CompactThread = func() error {
				ctx, cancel := withRuntimeContext()
				defer cancel()
				return runtimeController.CompactThread(ctx, threadReq)
			}
			rt.ResetThread = func() error {
				ctx, cancel := withRuntimeContext()
				defer cancel()
				return runtimeController.ResetThread(ctx, threadReq)
			}
		}
