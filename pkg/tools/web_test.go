...
// with cf-mitigated: challenge triggers a retry using the honest codex-claw User-Agent,
...
func TestWebFetchTool_CloudflareChallenge_RetryWithHonestUA(t *testing.T) {
	...
	// Second request must use the honest codex-claw user agent
	if !strings.Contains(receivedUAs[1], "codex-claw") {
		t.Errorf("retry request UA = %q, want it to contain 'codex-claw'", receivedUAs[1])
	}
}
