...
func TestCreateProviderFromConfig_CodexAppServer(t *testing.T) {
	previous := newCodexAppServerRunner
	newCodexAppServerRunner = func(string, int) codexAppServerRunner {
		return &fakeCodexAppServerRunner{}
	}
	t.Cleanup(func() {
		newCodexAppServerRunner = previous
	})

	cfg := &config.ModelConfig{
		ModelName: "test-codex-app-server",
		Model:     "codex/gpt-5.4",
		Workspace: t.TempDir(),
	}
	...
	if _, ok := provider.(*CodexAppServerProvider); !ok {
		t.Fatalf("expected *CodexAppServerProvider, got %T", provider)
	}
}
